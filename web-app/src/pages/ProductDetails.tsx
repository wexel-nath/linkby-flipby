...
import { mockProducts, mockUsers, mockOffers } from "@/data/mockData"
...
const ProductDetails = () => {
  ...
  const isReservedForAnotherBuyer = product.status === "Reserved" && product.reservedForBuyerId !== user?.id
          ...
          <div className="space-y-6">
            <div>
              <div className="mb-2 flex items-center gap-2">
                ...
                <Badge
                  variant={
                    product.status === "Sold" ? "destructive" : product.status === "Reserved" ? "secondary" : "default"
                  }
                >
                  {product.status}
                </Badge>
            ...
            {user && isBuyer && product.status === "Available" && (
              <div className="space-y-2">
                ...
                {!hasActiveOffer && (
                  <Button variant="outline" className="w-full" onClick={() => setShowOfferDialog(true)}>
                    Make Counter Offer
                  </Button>
                )}
              </div>
                        ...
                        >
                          <div>
                            <p className="font-medium">
                              ...
                              <span className="ml-2 text-sm text-muted-foreground">({offer.offerBy})</span>
                            </p>
                            <p className="text-sm text-muted-foreground">{offer.timestamp.toLocaleString()}</p>
                          ...
                          {!offer.accepted && isSeller && offer.offerBy === "buyer" && (
                            <div className="flex gap-2">
                              ...
                              <Button size="sm" variant="outline" onClick={() => setShowOfferDialog(true)}>
                                Counter
                              </Button>
                            </div>
                          )}
                          {!offer.accepted && isBuyer && offer.offerBy === "seller" && offer.buyerId === user?.id && (
                            <div className="flex gap-2">
                              <Button size="sm" onClick={handleAcceptOffer}>
                                Accept
                              </Button>
